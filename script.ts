{
  ...
  const generateTags = (): void => {
    ...
    const tagList = document.querySelector(selectorTagsList) as HTMLLinkElement;
  ...
  const generateAuthors = (): void => {
    ...
    for (let article of articles) {
      ...
      const articleAuthor = article.querySelector(selectorArticleAuthor) as HTMLLinkElement;
